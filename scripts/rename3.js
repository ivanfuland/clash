 ...
 *
 *** 过滤参数 rename3 新增
 * [ptype=]  按协议过滤，多个协议用 + 连接，例如 ptype=vless+ss
 * [keep=]   按地区过滤，多个地区用 + 连接，例如 keep=香港+台湾+新加坡+日本+韩国+美国+英国
 *           支持“日韩”，会展开为 日本+韩国
 * [drop=]   删除包含关键词的节点，多个关键词用 + 连接，例如 drop=官网+试用
 * [dedupe=] 去重，可选 server-port-type、server-port、name
 * [max=]    每个地区最多保留 N 个；没有质量依据时不建议使用
 * [maxmap=] 每个地区单独限额，例如 maxmap=香港:2,日本:2,美国:2
 * [prefer=] 同地区限额前优先排序关键词，例如 prefer=原生+家宽+直连
 */
...
const PTYPE = inArg.ptype == undefined ? "" : decodeArg(inArg.ptype),
  KEEP = inArg.keep == undefined ? "" : decodeArg(inArg.keep),
  DROP = inArg.drop == undefined ? "" : decodeArg(inArg.drop),
  DEDUPE = inArg.dedupe == undefined ? "" : decodeArg(inArg.dedupe),
  MAX = inArg.max == undefined ? "" : decodeArg(inArg.max),
  MAXMAP = inArg.maxmap == undefined ? "" : decodeArg(inArg.maxmap),
  PREFER = inArg.prefer == undefined ? "" : decodeArg(inArg.prefer);
// prettier-ignore
const FG = ['🇭🇰','🇲🇴','🇹🇼','🇯🇵','🇰🇷','🇸🇬','🇺🇸','🇬🇧','🇫🇷','🇩🇪','🇦🇺','🇦🇪','🇦🇫','🇦🇱','🇩🇿','🇦🇴','🇦🇷','🇦🇲','🇦🇹','🇦🇿','🇧🇭','🇧🇩','🇧🇾','🇧🇪','🇧🇿','🇧🇯','🇧🇹','🇧🇴','🇧🇦','🇧🇼','🇧🇷','🇻🇬','🇧🇳','🇧🇬','🇧🇫','🇧🇮','🇰🇭','🇨🇲','🇨🇦','🇨🇻','🇰🇾','🇨🇫','🇹🇩','🇨🇱','🇨🇴','🇰🇲','🇨🇬','🇨🇩','🇨🇷','🇭🇷','🇨🇾','🇨🇿','🇩🇰','🇩🇯','🇩🇴','🇪🇨','🇪🇬','🇸🇻','🇬🇶','🇪🇷','🇪🇪','🇪🇹','🇫🇯','🇫🇮','🇬🇦','🇬🇲','🇬🇪','🇬🇭','🇬🇷','🇬🇱','🇬🇹','🇬🇳','🇬🇾','🇭🇹','🇭🇳','🇭🇺','🇮🇸','🇮🇳','🇮🇩','🇮🇷','🇮🇶','🇮🇪','🇮🇲','🇮🇱','🇮🇹','🇨🇮','🇯🇲','🇯🇴','🇰🇿','🇰🇪','🇰🇼','🇰🇬','🇱🇦','🇱🇻','🇱🇧','🇱🇸','🇱🇷','🇱🇾','🇱🇹','🇱🇺','🇲🇰','🇲🇬','🇲🇼','🇲🇾','🇲🇻','🇲🇱','🇲🇹','🇲🇷','🇲🇺','🇲🇽','🇲🇩','🇲🇨','🇲🇳','🇲🇪','🇲🇦','🇲🇿','🇲🇲','🇳🇦','🇳🇵','🇳🇱','🇳🇿','🇳🇮','🇳🇪','🇳🇬','🇰🇵','🇳🇴','🇴🇲','🇵🇰','🇵🇦','🇵🇾','🇵🇪','🇵🇭','🇵🇹','🇵🇷','🇶🇦','🇷🇴','🇷🇺','🇷🇼','🇸🇲','🇸🇦','🇸🇳','🇷🇸','🇸🇱','🇸🇰','🇸🇮','🇸🇴','🇿🇦','🇪🇸','🇱🇰','🇸🇩','🇸🇷','🇸🇿','🇸🇪','🇨🇭','🇸🇾','🇹🇯','🇹🇿','🇹🇭','🇹🇬','🇹🇴','🇹🇹','🇹🇳','🇹🇷','🇹🇲','🇻🇮','🇺🇬','🇺🇦','🇺🇾','🇺🇿','🇻🇪','🇻🇳','🇾🇪','🇿🇲','🇿🇼','🇦🇩','🇷🇪','🇵🇱','🇬🇺','🇻🇦','🇱🇮','🇨🇼','🇸🇨','🇦🇶','🇬🇮','🇨🇺','🇫🇴','🇦🇽','🇧🇲','🇹🇱']
// prettier-ignore
...
const nameblnx = /(Super|高倍|(?!1)2+(x|倍)|ˣ²|ˣ³|ˣ⁴|ˣ⁵|ˣ¹⁰)/i;
const namenx = /(Super|高倍|(?!1)(0\.|\d)+(x|倍)|ˣ²|ˣ³|ˣ⁴|ˣ⁵|ˣ¹⁰)/i;
const keya =
...
function normalizeNameForFilter(name) {
  ...
  Object.keys(rurekey).forEach((ikey) => {
    const reg = resetRegex(rurekey[ikey]);
    if (reg.test(n)) {
      resetRegex(reg);
      n = n.replace(reg, ikey);
    }
  });
  return n;
}

function getRegionIndexByName(name) {
  const n = normalizeNameForFilter(name);
  const lists = [ZH, FG, QC, EN];
  for (const arr of lists) {
    for (let i = 0; i < arr.length; i++) {
      const key = arr[i];
      if (key && n.includes(key)) return i;
    }
  }
  return -1;
}

function getRegionZhByName(name) {
  const idx = getRegionIndexByName(name);
  return idx >= 0 ? ZH[idx] : "";
}

function buildKeepSet() {
  const items = splitArg(KEEP);
  ...
  items.forEach((item) => {
    ...
    const idx = getRegionIndexByName(item);
    if (idx >= 0) {
      set.add(ZH[idx]);
    } else {
      set.add(item);
...
function parseMaxMap() {
  const map = {};
  splitArg(MAXMAP).forEach((item) => {
    const parts = item.split(/[:：]/);
    if (parts.length !== 2) return;
    const key = parts[0].trim();
    const num = parseInt(parts[1].trim(), 10);
    if (key && Number.isFinite(num)) map[key] = num;
  });
  return map;
}
...
function getDedupeKey(p) {
  const mode = DEDUPE || "";
  if (mode === "name") return String(p.name || "");
  if (mode === "server-port") return [p.server, p.port].join("|");
  return [p.server, p.port, p.type].join("|");
}

function getPreferScore(name) {
  const keys = splitArg(PREFER);
  if (!keys.length) return 0;
  return keys.reduce((score, key, index) => {
    return String(name || "").includes(key) ? score + (1000 - index) : score;
  }, 0);
}

function rename3Filter(pro) {
  if (!PTYPE && !KEEP && !DROP && !DEDUPE && !MAX && !MAXMAP && !PREFER) {
    return pro;
  }

  const ptypes = splitArg(PTYPE).map((i) => i.toLowerCase());
  const drops = splitArg(DROP);
  const keepSet = buildKeepSet();
  const maxNum = parseInt(MAX, 10);
  const hasMax = Number.isFinite(maxNum);
  const maxMap = parseMaxMap();
  const hasMaxMap = Object.keys(maxMap).length > 0;

  let list = pro.map((p, index) => ({
    p,
    index,
    name: String(p.name || ""),
    region: getRegionZhByName(p.name),
  }));
  ...
  list = list.filter((item) => {
    ...
    if (clear && nameclear.test(item.name)) return false;
    if (ptypes.length && !ptypes.includes(ptype)) return false;
    if (drops.length && drops.some((kw) => item.name.includes(kw))) return false;
    if (keepSet && (!item.region || !keepSet.has(item.region))) return false;

    return true;
  });

  if (PREFER && (hasMax || hasMaxMap)) {
    list = list
      .map((item) => ({ ...item, score: getPreferScore(item.name) }))
  ...
  if (hasMax || hasMaxMap) {
    ...
    list = list.filter((item) => {
      ...
      const limit = maxMap[item.region] !== undefined ? maxMap[item.region] : maxNum;
      if (!Number.isFinite(limit)) return true;

      count[item.region] = count[item.region] || 0;
      if (count[item.region] >= limit) return false;
      count[item.region]++;
      return true;
    });
  }
